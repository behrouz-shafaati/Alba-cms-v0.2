 ...
 */
export async function generateUniqueSectionSlug(
  ...
  if (templatePartId !== '') {
    const findedSectionBySlug = await templatePartCtrl.findOne({
      filters: { slug: baseSlug },
    })
    if (findedSectionBySlug && findedSectionBySlug.id == templatePartId) {
