...
class controller extends baseController {
   ...
   *
   * @remarks
   * This method is part of the formController class extended of the main parent class baseController.
   *
   * @param service - formService
   *formCtrl
   * @beta
   */
  ...
  async existSlug(slug: string): Promise<boolean> {
    ...
    console.log('#7d736 Form count: ', count)
    if (count > 0) return true
    return false
  }
}

const formCtrl = new controller(new formService(formSchema))
