...
import { deleteProductTagsAction } from '../../actions'
...
export const CellAction: React.FC<CellActionProps> = ({ data }) => {
  ...
  const canEdit = authorize(
    userRoles,
    data?.user.id !== user?.id ? 'productTag.edit.any' : 'productTag.edit.own',
  )
  ...
  const onConfirm = async () => {
    setLoading(true)
    await deleteProductTagsAction([data.id])
    router.refresh()
    setOpen(false)
    setLoading(false)
  }
    ...
    <>
      ...
      <DropdownMenu modal={false}>
        ...
        <DropdownMenuContent align="start">
          ...
          {canEdit && (
            <DropdownMenuItem
              onClick={() => router.push(`/dashboard/product-tags/${data.id}`)}
            >
