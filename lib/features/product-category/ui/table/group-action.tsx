...
import { deleteProductCategorysAction } from '../../actions'
...
export default function GroupAction({ table, items }: GroupActionProps) {
  ...
  for (const item of items) {
    const canDeleteItem = authorize(
      userRoles,
      item.user.id !== user?.id ? 'category.delete.any' : 'category.delete.own',
    )
    if (!canDeleteItem) {
      canDelete = false
      break
  ...
  const onDelete = async () => {
    try {
      setLoading(true)
      await deleteProductCategorysAction(items.map((i) => i.id))
      router.refresh()
