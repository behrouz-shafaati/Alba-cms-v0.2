...
import { SMSSettings } from './interface'
import { FormActionState } from '@/lib/types'
import authorize from '@/lib/utils/authorize'

const METADATA_KEY = 'sms'

const FormSchema = z.object({
  apiKey: z.string({}).nullable().optional(),
  verifyPatternCode: z.string({}).nullable().optional(),
...
async function sanitizeSettingsData(validatedFields: any) {
  ...
  const settingsPayload: SMSSettings = validatedFields
  const farazsms = {
    apiKey: settingsPayload?.apiKey,
    verifyPatternCode: settingsPayload?.verifyPatternCode,
    from_number: settingsPayload?.from_number,
  }
  const params = {
    value: { farazsms },
  }
