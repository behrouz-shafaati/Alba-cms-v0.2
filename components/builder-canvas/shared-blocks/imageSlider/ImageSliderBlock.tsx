...
import LeftSliderButton from '@/components/other/ui/left-slider-button'
import RightSliderButton from '@/components/other/ui/right-slider-button'
  ...
  const images = content.map((img: FileDetails, i: number) => {
    ...
    return img.href ? (
      <div className="relative min-w-full" key={i}>
        <LinkAlba href={img.href} target="_blank" rel="noopener noreferrer">
          {imageElement}
        </LinkAlba>
      </div>
    ) : (
